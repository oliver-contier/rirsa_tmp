...
def rirsa_contrasts(nsubs=6):
    ...
    regressor_names = ['pretraining', 'posttraining', 'intercept'] + \
                      ['sub{:02d}'.format(sub) for sub in range(1, nsubs + 1)]

    # list of contrasts
    contrast_list = [
        ('overall_modelfit', 'T', regressor_names, [0, 0, 1] + [0] * nsubs),
        ('posttrain_minus_pretrain', 'T', regressor_names, [1, -1, 0] + [0] * nsubs),
        ('pretrain_minus_posttrain', 'T', regressor_names, [-1, 1, 0] + [0] * nsubs)
...
def rirsa_make_design(nsubs=6):
    ...
    pretraining = [1, 1, 1, 1, -1, -1, -1, -1] + [0] * 8  # trained > untrained in session 1
    posttraining = [0] * 8 + [1, 1, 1, 1, -1, -1, -1, -1]  # trained > untrained in session 2
    intercept = [1] * 16  # model intercept # TODO: don't need to need demean option in randomise

    # create design dict which contains each regressor repeated for each subject
    design_dict = {'pretraining': pretraining * nsubs,
                   'posttraining': posttraining * nsubs,
                   'intercept': intercept * nsubs}
